...
public class MainActivity extends AppCompatActivity {
    ...
    private void retriewData() {
        pbQuote.setVisibility(View.VISIBLE);

        APIRequestData ARD = RetroServer.connectRetrofit().create(APIRequestData.class);

        Call<List<QuoteModel>> retriewProcess = ARD.ardRetrieve();

        retriewProcess.enqueue(new Callback<List<QuoteModel>>() {
            @Override
            ...
            public void onFailure(Call<List<QuoteModel>> call, Throwable t) {
                Toast.makeText(MainActivity.this, "Cannection Error! Cannot Reach Server! ", Toast.LENGTH_SHORT).show();
                pbQuote.setVisibility(View.GONE);
